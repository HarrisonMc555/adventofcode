...
class Unit:
    ...
    def remove(self):
        if self.grid[self.row][self.col].is_occupied():
    ...
    def tick(self, units, nothing_changed_last_time, elf_power):
        if not nothing_changed_last_time:
            did_move = self.move(units)
        else:
            did_move = False
        killed_unit = self.attack(units, nothing_changed_last_time, elf_power)
    ...
    def attack(self, units, nothing_changed_last_time, elf_power):
        ...
        if not target:
            self.last_target = None
            return None
        is_dead = target.attacked(self.get_attack_power(elf_power))
    ...
    def get_attack_power(self, elf_power):
        # print('get_attack_power:', self)
        if self.team == Team.ELF:
            # print('elf power! power:', elf_power)
            return elf_power
        return Unit.GOBLIN_ATTACK_POWER
...
def solve(units):
    ...
    num_elves = len([u for u in units if u.team == Team.ELF])
    while True:
        print('Running with elf_power:', elf_power)
        num_rounds, winner = run_combat(units, elf_power)
        print('num_rounds: {}, winner: {}'.format(num_rounds, winner))
        if winner != Team.ELF or len(units) != num_elves:
            units = set(all_units)
            # print(', '.join(str(u) for u in units))
            # print(create_grid_string(next(iter(units)).grid, units, show_hp=True))
            reset(units)
...
def run_combat(units, elf_power):
    num_rounds = 0
    game_done = False
    ...
    # print(', '.join(str(u) for u in units))
    return num_rounds, next(iter(units)).team
