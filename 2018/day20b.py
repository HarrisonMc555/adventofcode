...
def solve(regex):
    ...
    min_length = 1000
    return count_paths_longer_than(connections, min_length)
...
def count_paths_longer_than(connections, min_length):
    distances = find_distances(connections)
    return count(distances.values(),
                 lambda v: v >= min_length and v != math.inf)

def count(enumerable, fun):
    return sum(1 for v in enumerable if fun(v))

def find_distances(connections):
    start_pos = (0, 0)
    positions = [start_pos]
