...
def get_least_gold(shop_info, boss_stats):
    ...
    wins = lambda c: is_winning_combination(boss_stats, c)
    winning_combinations = filter(wins, get_combinations(shop_info))
    return min(calc_price(c) for c in winning_combinations)
