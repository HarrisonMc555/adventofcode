...
def main():
    ...
    print(max(reindeer.distance for reindeer in reindeer_list))
...
class Reindeer:
    ...
    def new(name, speed, fly_time, rest_time):
        return Reindeer(name, speed, fly_time, rest_time, 0, True, 0)
...
class Test(unittest.TestCase):
    def test_examples(self):
        ...
        max_num_steps = max(TIME_TO_REINDEER_DISTANCES.keys())
        for step in range(1, max_num_steps + 1):
            run_step(reindeer_list)
            if step in TIME_TO_REINDEER_DISTANCES:
                for name, distance in TIME_TO_REINDEER_DISTANCES[step]:
                    reindeer = [r for r in reindeer_list if r.name == name][0]
                    self.assertEqual(reindeer.distance, distance,
                                     f'step: {step}, reindeer: {name}, distance: {distance}')
